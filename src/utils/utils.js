...
export function getTokenURI(uri, infuraDedicatedGateway) {
  return fetch(getValidIPFSUrl(uri, infuraDedicatedGateway), { method: "POST" })
...
export function getValidImageUrl(imageUrl, infuraDedicatedGateway) {
  ...
  return fetch(`${infuraDedicatedGateway}/api/v0/cat?arg=${imageUrl}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded"
    },
  })
    .then(response => response.blob()) // Chuyển dữ liệu về dạng Blob
    .then(blob => {
      const objectURL = URL.createObjectURL(blob); // Tạo Object URL từ Blob
      console.log("Object URL:", objectURL);
      return objectURL; // Trả về URL
    })
    .catch(error => {
...
export function getValidIPFSUrl(url, infuraDedicatedGateway) {
  ...
  return `${infuraDedicatedGateway}/api/v0/cat?arg=/${url}`
}
